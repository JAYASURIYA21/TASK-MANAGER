    ...
    <div className="flex flex-1 flex-col bg-gray-100 border rounded-2xl p-5 space-y-4 ">
      <div className="flex flex-row justify-between h-fit items-center min-w-full w-full">
        ...
        <div className="flex flex-row justify-between border rounded-lg p-2 space-x-8 h-fit items-center text-sm font-medium text-gray-500 bg-white">
          <p className={`hover:text-green-400 hover:cursor-pointer ${filter==='all' && "text-green-400"}`} onClick={()=>handleFilter('all')}>All</p>
          <p className={`hover:text-green-400 hover:cursor-pointer ${filter==='low' && "text-green-400"}`} onClick={()=>handleFilter('low')}>Low</p>
          <p className={`hover:text-green-400 hover:cursor-pointer ${filter==='medium' && "text-green-400"}`} onClick={()=>handleFilter('medium')}>Medium</p>
          <p className={`${filter==='high' && "text-green-400"} hover:text-green-400 hover:cursor-pointer`} onClick={()=>handleFilter('high')}>High</p>
        </div>
      </div>

      <div className=" flex flex-1">
        <div className="grid md:grid-cols-2 mt-3 h-[65vh]   sm:grid-cols-1 lg:grid-cols-3 grid-cols-1 gap-4  overflow-y-scroll w-full">
          {
            Alltask && Alltask.length>0?
             Alltask.map((task,index)=>{
              ...
              const backdrop=task.completed===true?"bg-green-200":"bg-white"
              const date=moment(task.dueDate).format('dddd');
              if(filter==='all' ||  task.priority===filter)
              return(
              <div key={index} className={`overflow-hidden h-44  lg:h-60 max-w-full   p-3 ${backdrop} border rounded-lg flex flex-col justify-between`}>
              <div >
            ...
            : null
              ...
              <div onClick={()=>setCreatePopUp(!createPopUp)} className="overflow-hidden lg:h-60 max-w-full p-3 text-center border-2 text-gray-500 border-dotted border-black rounded-lg h-44  hover:bg-gray-400 flex items-center justify-center font-medium
              text-xl">
                <div>
                  Add new Task
                </div>
