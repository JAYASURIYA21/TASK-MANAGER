  ...
  return (
    ...
    <div className="flex flex-1 flex-col bg-gray-100 border rounded-2xl p-5 space-y-4">
      <div className="flex flex-row justify-between h-fit items-center min-w-full w-full">
        <h2 className="text-2xl font-bold">Onprogress Tasks</h2>
        <div className="flex flex-row justify-between border rounded-lg p-2 space-x-8 h-fit items-center text-sm font-medium text-gray-500 bg-white">
          <p className={`${filter==='all' && "text-green-400"} hover:text-green-400 hover:cursor-pointer`} onClick={()=>setFilter('all')}>All</p>
          <p className={`hover:text-green-400 hover:cursor-pointer ${filter==='low' && "text-green-400"}`} onClick={()=>setFilter('low')}>Low</p>
      ...
      <div className=" flex flex-1">
        <div className="grid md:grid-cols-2 mt-5  sm:grid-cols-1 lg:grid-cols-3 grid-cols-1 gap-4 overflow-y-scroll h-[65vh]  overflow-hidden w-full">
          {
            Alltask && Alltask.length>0?
             Alltask.map((task,index)=>{
              ...
              if(task.completed===false){
                if(filTask==='all' || filTask===task.priority)
              return(
              <div key={task._id} className={`h-44  overflow-hidden lg:h-60 max-w-full  p-3 ${backdrop} border rounded-lg flex flex-col justify-between`}>
              <div >
